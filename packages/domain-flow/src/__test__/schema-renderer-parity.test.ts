/**
 * Schema ↔ renderer parity: the drift lock.
 *
 * Two renderings of the same command ship in this package and could disagree:
 * the hand-written `renderFlow` and the schema-driven `createSchemaRenderer`
 * over the same schemas, with nothing asserting they agree. This asserts it for
 * every command × language, on a fully-populated node and on a minimal one.
 *
 * `renderFlow`'s output is frozen (downstream depends on it), so a mismatch is
 * fixed by teaching the SCHEMA what the renderer does — never the other way
 * round. The schema path is what an extension command renders through, so a
 * schema that disagrees with the renderer means extensions render an
 * unidiomatic surface.
 *
 * The SOV round-trip block below is not decoration: `submit` and `transform`
 * declared the goal/instrument BEFORE the object in `sovPosition`, while
 * `renderFlow` wrote the object first. Since `sovPosition` drives pattern
 * generation as well as rendering, that made flow's own Japanese, Korean and
 * Turkish output unparseable by flow's own parser. Parity surfaced it.
 */

import { describe, it, expect } from 'vitest';
import { createSchemaRenderer, type SemanticNode } from '@lokascript/framework';
import { allSchemas, allProfiles, createFlowDSL, renderFlow } from '../index';

const schemaRenderer = createSchemaRenderer(allSchemas, allProfiles);
const flow = createFlowDSL();
const LANGUAGES = allProfiles.map(p => p.code);
const SOV_LANGUAGES = ['ja', 'ko', 'tr'];

/** One English example per command, exercising every role the command has. */
const EXAMPLES: Record<string, string> = {
  fetch: 'fetch /api/users as json into #list',
  poll: 'poll /api/status every 5s as json into #status',
  stream: 'stream /api/events as sse into #feed',
  submit: 'submit #form to /api/orders as json',
  transform: 'transform data with uppercase',
  enter: 'enter /api',
  follow: 'follow orders item 42',
  perform: 'perform createOrder with #checkout',
  capture: 'capture message as confirmationText',
};

/** Drop every optional role, leaving only what the schema requires. */
function stripOptionalRoles(node: SemanticNode): SemanticNode {
  const schema = allSchemas.find(s => s.action === node.action);
  const required = new Set(schema?.roles.filter(r => r.required).map(r => r.role) ?? []);
  const roles = new Map([...node.roles].filter(([role]) => required.has(role)));
  return { ...node, roles };
}

describe('createSchemaRenderer agrees with renderFlow', () => {
  for (const schema of allSchemas) {
    const example = EXAMPLES[schema.action];
    if (!example) {
      it(`${schema.action} has an example to compare`, () => {
        expect.fail(`No example for "${schema.action}" — add one to EXAMPLES.`);
      });
      continue;
    }

    const full = flow.parse(example, 'en');
    const minimal = stripOptionalRoles(full);

    it(`${schema.action}'s example populates every role`, () => {
      const missing = schema.roles.map(r => r.role).filter(role => !full.roles.has(role));
      expect(missing).toEqual([]);
    });

    for (const language of LANGUAGES) {
      it(`${schema.action} × ${language} — all roles populated`, () => {
        expect(schemaRenderer.render(full, language)).toBe(renderFlow(full, language));
      });

      it(`${schema.action} × ${language} — required roles only`, () => {
        expect(schemaRenderer.render(minimal, language)).toBe(renderFlow(minimal, language));
      });
    }
  }
});

describe('SOV output round-trips through the parser', () => {
  // `sovPosition` orders the generated pattern AND the rendered surface. When
  // the two disagree the renderer emits text its own parser rejects, which no
  // renderX-only test can see. Rendering required roles only keeps the surface
  // to what the pattern must match.
  for (const action of ['submit', 'transform']) {
    const full = flow.parse(EXAMPLES[action], 'en');
    const minimal = stripOptionalRoles(full);

    for (const language of SOV_LANGUAGES) {
      it(`${action} × ${language} — rendered text parses back to the same roles`, () => {
        const surface = renderFlow(minimal, language);
        expect(surface).toBeTruthy();

        const reparsed = flow.parse(surface as string, language);
        expect(reparsed.action).toBe(action);
        expect([...reparsed.roles.keys()].sort()).toEqual([...minimal.roles.keys()].sort());
      });
    }
  }
});

describe('keyword tables agree with the language profiles', () => {
  // renderFlow reads its own COMMAND_KEYWORDS table; the schema renderer reads
  // the profiles. A keyword that drifts between them would show up above as an
  // opaque string diff, so check it directly for a readable failure.
  for (const schema of allSchemas) {
    it(`${schema.action} has a profile keyword in every supported language`, () => {
      const missing = LANGUAGES.filter(code => {
        const profile = allProfiles.find(p => p.code === code);
        return !profile?.keywords[schema.action]?.primary;
      });
      expect(missing).toEqual([]);
    });
  }
});
