 ...
 * structural step keywords. Nothing per-language is hand-authored here anymore.
 *
 * BDD's roles (target/state/action_type/value/assertion/expected_value/content)
 * don't collide with any semantic role, so the slices' role markers never leak
 * into generation — no vocab suppression is needed. Role markers are specified
 * via `markerOverride` on each schema role (in schemas/index.ts).
 */

import { buildPatternProfile } from '@lokascript/framework';
import type { PatternGenLanguageProfile } from '@lokascript/framework';
import { BDD_LANGUAGES } from '../vocab';

function profileFor(code: string): PatternGenLanguageProfile {
  const { slice, vocab } = BDD_LANGUAGES[code];
